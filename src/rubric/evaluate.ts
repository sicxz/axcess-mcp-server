...
import { TypographySet, EvaluationReport, ElementEvaluation, RubricIssue } from './types.js';
import { evaluateTypographyElement } from './typography.js';
import { BRINGHURST_LUPTON_SET_CHECKS } from './bringhurst_lupton.js';

export function evaluateTypographySet(input: TypographySet): EvaluationReport {
  // Infer body font size from the set for scale-ratio checks
  const bodyElements = input.elements.filter((e) =>
    ['body', 'caption'].includes(e.element_type)
  );
  const bodyFontSize =
  ...
  const elementEvaluations: ElementEvaluation[] = input.elements.map((el) =>
    evaluateTypographyElement(el, bodyFontSize)
  );

  // Run set-level checks (Bringhurst/Lupton rules that require seeing all elements)
  const setLevelIssues: RubricIssue[] = BRINGHURST_LUPTON_SET_CHECKS
    .map((fn) => fn(input))
  ...
  if (setLevelIssues.length > 0) {
    const setEval: ElementEvaluation = {
