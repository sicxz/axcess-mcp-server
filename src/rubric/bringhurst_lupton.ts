...
export function brng_measureTooWide(el: TypographyElement): RubricIssue | null {
  if (!['body', 'caption'].includes(el.element_type)) return null;
  if (!el.content_length) return null;
  ...
  if (el.content_length > 80) {
    return {
      ...
      what_scanners_miss:
        'No accessibility scanner checks line measure against typographic standards. WCAG 1.4.8 (AAA) recommends max 80 characters but does not appear in AA audits.',
      ...
      recommendation:
        "Constrain body text columns to approximately 65–66 characters per line (Bringhurst ideal). Use max-width: 65ch on text containers. For justified text, aim for 60–66 characters.",
...
export function brng_measureTooNarrow(el: TypographyElement): RubricIssue | null {
  if (!['body'].includes(el.element_type)) return null;
  if (!el.content_length) return null;
  ...
  if (el.content_length < 40) {
    return {
      ...
      what_scanners_miss:
        'Narrow measure is never flagged by accessibility tools. It is a craft problem, not a compliance problem.',
      ...
      recommendation:
        'Widen the text column or reduce font size to achieve a measure of 45–65 characters. For multi-column layouts, 40–50 characters per column is acceptable.',
      passes_wcag_minimum: true,
      passes_design_judgment: false,
      measured_value: `~${el.content_length} chars`,
    };
...
export function brng_leadingToMeasure(el: TypographyElement): RubricIssue | null {
  if (!['body'].includes(el.element_type)) return null;
  if (!el.content_length) return null;
  ...
  if (veryWideMeasure && el.line_height < 1.6) {
    return {
      ...
      what_scanners_miss:
        'The relationship between line length and line height is invisible to automated tools. A scanner sees line-height: 1.4 and passes it. It cannot know the column is 95 characters wide.',
      ...
      recommendation:
        'Increase line-height to 1.6–1.7 for this column width, or reduce the column width to 65 characters and drop leading to 1.5.',
      passes_wcag_minimum: true,
      passes_design_judgment: false,
  ...
  if (wideMeasure && el.line_height < 1.5) {
    return {
      ...
      wcag_reference: '§2.2.1 Elements of Typographic Style',
      what_scanners_miss:
        'The measure-to-leading relationship is not checkable by automated tools.',
      ...
      recommendation:
        'Increase line-height to 1.5–1.6 for this measure.',
      passes_wcag_minimum: true,
      passes_design_judgment: false,
      measured_value: `line-height: ${el.line_height}, measure: ~${el.content_length} chars`,
    };
...
export function brng_opticalSizeSmall(el: TypographyElement): RubricIssue | null {
  const isSmall = el.font_size < 13; // ~10pt
  if (!isSmall) return null;
  if (!el.letter_spacing || el.letter_spacing >= 0) return null;
  ...
  return {
    ...
    what_scanners_miss:
      'Optical sizing is a craft concept with no automated equivalent. WCAG 1.4.12 checks that spacing can be overridden, not that defaults are optically correct at a given size.',
    ...
    recommendation:
      'Remove negative tracking on small type. At sizes below 13px, increase letter-spacing to 0.02–0.04em to compensate for optical compression. Use a typeface designed for text sizes, not a display face scaled down.',
    passes_wcag_minimum: true,
    passes_design_judgment: false,
...
// at large sizes."
export function brng_displayTracking(el: TypographyElement): RubricIssue | null {
  const isDisplay = el.font_size >= 48; // ≈36pt+
  const isHeading = el.element_type === 'heading' || el.element_type === 'display';
  if (!isDisplay || !isHeading) return null;
  ...
  if (el.letter_spacing !== undefined && el.letter_spacing > 0.02) {
    return {
      ...
      what_scanners_miss:
        'No scanner evaluates display tracking. Positive letter-spacing technically aids readability by automated measures — but at display sizes it creates visual gaps that undermine the word shapes readers rely on for fast recognition.',
      ...
      recommendation:
        'Reduce letter-spacing to 0 or slightly negative (-0.01 to -0.02em) for display type above 48px. This tightens the word shapes and improves visual color at large sizes.',
      passes_wcag_minimum: true,
      passes_design_judgment: false,
      measured_value: `${el.font_size}px, letter-spacing: +${el.letter_spacing}em`,
    };
...
export function brng_headingScale(
  ...
  const minimumRatios: Record<number, number> = {
  ...
  if (level <= 3 && ratio < minimumRatio) {
    return {
      ...
      what_scanners_miss:
        'Scanners verify heading markup exists. They cannot evaluate whether the visual scale relationship communicates the hierarchy that the markup implies.',
...
// all-caps headings.
export function lupt_capsTracking(el: TypographyElement): RubricIssue | null {
  ...
  if (tracking < 0.05) {
    ...
    return {
      ...
      what_scanners_miss:
        'No automated tool checks whether all-caps text has been tracked appropriately. WCAG 1.4.12 checks that spacing can be overridden; it does not require the default to reflect typographic practice.',
...
export function lupt_weakHierarchy(el: TypographyElement): RubricIssue | null {
  if (el.element_type !== 'heading' || !el.heading_level) return null;
  if (el.heading_level > 3) return null;
  ...
  if (!hasWeightContrast && !hasScaleContrast) {
    return {
      ...
      what_scanners_miss:
        'Scanners confirm heading markup. They cannot evaluate whether the visual treatment communicates the structural role. A heading at 400 weight and 18px looks like body text regardless of its HTML tag.',
...
export function lupt_bodyWeight(el: TypographyElement): RubricIssue | null {
  if (!['body', 'caption'].includes(el.element_type)) return null;
  ...
  if (el.font_weight >= 700) {
    return {
      ...
      what_scanners_miss:
        'Bold body text passes all accessibility checks. No scanner evaluates typographic texture.',
      ...
      recommendation:
        'Use weight 400 (regular) for body text. Reserve 500–600 for lead text or introductory paragraphs. 700+ should appear only for inline emphasis — a word or phrase, not a paragraph.',
      passes_wcag_minimum: true,
      passes_design_judgment: false,
...
export function lupt_displayOverworked(el: TypographyElement): RubricIssue | null {
  if (!['heading', 'display'].includes(el.element_type)) return null;
  if (el.font_size < 48) return null;
  ...
  if (contrastVariables >= 3) {
    return {
      ...
      what_scanners_miss:
        'Over-designed display type passes all automated checks. The problem is not accessibility in the technical sense — it is legibility and visual hierarchy in the design sense.',
      ...
      recommendation:
        'Choose one or two contrast variables for display type. Large size alone is often sufficient. If all-caps, reduce weight to 400–600. If heavy weight, use mixed case. Trust the scale to do the work.',
      passes_wcag_minimum: true,
      passes_design_judgment: false,
...
export function lupt_monotoneWeight(set: TypographySet): RubricIssue | null {
  if (set.elements.length < 2) return null;
  ...
  if (weights.size === 1 && hasHeading && hasBody) {
    ...
    return {
      ...
      wcag_reference: 'Thinking with Type §Hierarchy / WCAG 1.3.1',
      what_scanners_miss:
        'Weight hierarchy is invisible to automated tools. A page can have perfect heading structure in markup with zero visual differentiation between heading and body weight.',
      ...
      recommendation:
        `Introduce weight contrast: set headings to 600–700 and body to 400. Weight is the most reliable single variable for communicating text hierarchy, more robust than size or color alone.`,
      passes_wcag_minimum: true,
      passes_design_judgment: false,
...
export function brng_sizeMonotony(set: TypographySet): RubricIssue | null {
  if (set.elements.length < 2) return null;
  ...
  if (sizes.size === 1 && hasHeading && hasBody) {
    ...
    return {
      ...
      what_scanners_miss:
        'Scanners verify heading markup. They cannot detect that all elements are visually indistinguishable in size.',
      ...
      recommendation:
        'Establish a modular scale. A minimum: body at 16px, H3 at 20px (1.25×), H2 at 24px (1.5×), H1 at 32px (2×). For a more refined system, use a consistent ratio throughout — major third (×1.25), perfect fourth (×1.333), or golden section (×1.618).',
