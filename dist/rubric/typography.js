...
import { BRINGHURST_LUPTON_ELEMENT_CHECKS, brng_headingScale, } from './bringhurst_lupton.js';
import { contrastRatio, isLargeText, WCAG_AA_NORMAL, WCAG_AA_LARGE, WCAG_AAA_NORMAL, } from './color.js';
// ─── Score Deductions ───────────────────────────────────────────────────────
const DEDUCTIONS = {
    critical: 25,
    major: 15,
...
// ─── Individual Checks ──────────────────────────────────────────────────────
function checkContrastWithWeight(el) {
    ...
    const thinWeightRisk = el.font_weight < 400 && ratio < WCAG_AAA_NORMAL && !large;
    if (!passesWcag) {
        return {
    ...
    if (thinWeightRisk) {
        return {
            ...
            what_scanners_miss: 'Automated tools only check the contrast ratio number. They cannot evaluate how font weight affects perceived legibility. A 4.6:1 ratio in weight-300 reads harder than the same ratio in weight-700.',
...
function checkMinimumSize(el) {
    ...
    if (isBody && el.font_size < 16) {
        return {
            ...
            what_scanners_miss: 'WCAG 1.4.4 only requires text to be resizable, not that it starts at a readable size. Scanners do not flag small base sizes.',
    ...
    if (isUI && el.font_size < 14) {
        return {
            ...
            severity: 'minor',
            wcag_reference: 'WCAG 1.4.4',
            what_scanners_miss: 'Scanners do not evaluate base font size, only zoom behavior.',
...
function checkLineHeight(el) {
    ...
    if (el.line_height < 1.4) {
        return {
            ...
            what_scanners_miss: 'WCAG 1.4.12 requires that text spacing can be overridden without loss of content. It does not require the default spacing to be adequate. Scanners check the ability to override, not the default value.',
    ...
    if (el.line_height > 2.2) {
        return {
            ...
            severity: 'minor',
            wcag_reference: 'WCAG 1.4.12',
            what_scanners_miss: 'Excessive line height is never flagged by automated tools — they only check minimums.',
...
function checkLineLength(el) {
    const isBody = ['body'].includes(el.element_type);
    if (!isBody || el.content_length === undefined)
        return null;
    ...
    if (el.content_length > 85) {
        return {
            ...
            what_scanners_miss: 'No automated tool checks line length for body text. WCAG AAA recommends no more than 80 characters per line but automated tools do not enforce this.',
...
function checkAllCaps(el) {
    if (el.text_transform !== 'uppercase')
        return null;
    const length = el.content_length ?? 0;
    if (length > 30) {
        const severity = length > 80 ? 'major' : 'minor';
        return {
            criterion_id: 'TYP-ALL-CAPS',
            criterion: 'Extended all-caps text — impedes readability',
            severity,
            wcag_reference: 'WCAG 1.4.8 (AAA)',
            what_scanners_miss: 'No automated scanner checks for all-caps text. It technically passes all WCAG criteria at AA level.',
...
function checkExtendedItalic(el) {
    if (el.font_style !== 'italic')
        return null;
    const length = el.content_length ?? 0;
    if (length > 80) {
        return {
            ...
            wcag_reference: 'WCAG 1.4.8 (AAA)',
            what_scanners_miss: 'No scanner flags extended italic use. It passes all automated checks.',
...
function checkLetterSpacing(el) {
    if (el.letter_spacing === undefined)
        return null;
    const isBody = ['body', 'caption'].includes(el.element_type);
    if (isBody && el.letter_spacing < -0.02) {
        return {
            ...
            wcag_reference: 'WCAG 1.4.12',
            what_scanners_miss: 'WCAG 1.4.12 checks that letter-spacing can be overridden, not that defaults are set appropriately. Negative tracking on body text is never flagged.',
...
function checkTextOnNonSolid(el) {
    if (!el.background_type || el.background_type === 'solid')
        return null;
    return {
        ...
        severity: 'major',
        wcag_reference: 'WCAG 1.4.3',
        what_scanners_miss: 'Contrast checkers compute an average or sampled color value. They cannot evaluate contrast across the full range of a gradient or image. A ratio that passes at the sampled point may fail elsewhere in the container.',
...
function checkHeadingContext(el) {
    ...
    if (el.font_size <= 18 && el.heading_level <= 3) {
        return {
            ...
            what_scanners_miss: 'Scanners verify that heading markup exists. They do not verify that heading styles are visually distinguishable from body text. A screen reader user benefits from heading markup regardless of visual size; a sighted user with cognitive disabilities relies on the visual hierarchy.',
...
// ─── Element Evaluator ──────────────────────────────────────────────────────
export function evaluateTypographyElement(el, bodyFontSize = 16) {
    const checks = [
